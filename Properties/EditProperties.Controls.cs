...
// EditProperties.aspx page.  The Row or RecordControl classes are the 
...
namespace IPv5.UI.Controls.EditProperties
{
...
// Base class for the LinkTableTableControlRow control on the EditProperties page.
...
public class BaseLinkTableTableControlRow : IPv5.UI.BaseApplicationRecordControl
{
        ...
        public virtual void SetContactID()
        {
            ...
            else
            {
                ...
                if (this.DataSource != null && this.DataSource.IsCreated)
                    ...
                    selectedValue = LinkTableTable.ContactID.DefaultValue;
        ...
        public virtual void GetContactID()
        {
         // Retrieve the value entered by the user on the ContactID ASP:DropDownList, and
            // save it into the ContactID field in DataSource DatabaseMM_IP1%dbo.LinkTable record.
            
            // Custom validation should be performed in Validate, not here.
            
            this.DataSource.Parse(MiscUtils.GetValueSelectedPageRequest(this.ContactID), LinkTableTable.ContactID);			
      ...
      // To customize, override this method in LinkTableTableControlRow.
              ...
              
        {
              ...
              try
              {
                    ...
                    url = url + "?RedirectStyle=" + (this.Page as BaseApplicationPage).Encrypt("Popup") + "&Target=" + (this.Page as BaseApplicationPage).Encrypt(this.ContactID.ClientID) + "&Formula=" + (this.Page as BaseApplicationPage).Encrypt("=  Contacts.FirstName + ' ' + Contacts.LastName")+ "&IndexField=" + (this.Page as BaseApplicationPage).Encrypt("ContactID");                      
...
// Base class for the LinkTableTableControl control on the EditProperties page.
...
public class BaseLinkTableTableControl : IPv5.UI.BaseApplicationTableControl
{
       ...
       public BaseLinkTableTableControl()
        ...
        protected virtual void Control_Init(object sender, System.EventArgs e)
        {
           ...
           // Setup the filter and search.
            ...
            {
                string initialVal = "";
                if  (this.InSession(this.ContactIDFilter1)) 				
                    initialVal = this.GetFromSession(this.ContactIDFilter1);
                ...
                if (initialVal != null && initialVal != "")		
                {
                    ...
                    this.ContactIDFilter1.Items.Add(new ListItem(initialVal, initialVal));
                        
                    this.ContactIDFilter1.SelectedValue = initialVal;
    ...
    // Setup default pagination settings.
       ...
       // Setup the sorting events.
            ...
            this.ContactIDFilter1.SelectedIndexChanged += new EventHandler(ContactIDFilter1_SelectedIndexChanged);
      ...
      public override void DataBind()
      {
        ...
        public virtual void ResetControl()
        {
            ...
            this.ContactIDFilter1.ClearSelection();
    ...
    IPv5.UI.Controls.EditProperties.PropertiesRecordControl propertiesRecordControlObj = (MiscUtils.FindControlRecursively(this.Page , "PropertiesRecordControl") as IPv5.UI.Controls.EditProperties.PropertiesRecordControl);
              ...
              {
              wc.iAND(LinkTableTable.PropertyID, BaseFilter.ComparisonOperator.EqualsTo, propertiesRecordControlObj.GetRecord().PropertyID.ToString());
              selectedRecordKeyValue.AddElement(LinkTableTable.PropertyID.InternalName, propertiesRecordControlObj.GetRecord().PropertyID.ToString());
              }
              else
              {
              wc.RunQuery = false;
      ...
      HttpContext.Current.Session["LinkTableTableControlWhereClause"] = selectedRecordKeyValue.ToXmlString();
            ...
            if (MiscUtils.IsValueSelected(this.ContactIDFilter1)) {
                        
                wc.iAND(LinkTableTable.ContactID, BaseFilter.ComparisonOperator.EqualsTo, MiscUtils.GetSelectedValue(this.ContactIDFilter1, this.GetFromSession(this.ContactIDFilter1)), false, false);
      ...
      string selectedRecordInPropertiesRecordControl = HttpContext.Current.Session["LinkTableTableControlWhereClause"] as string;
      ...
      String ContactIDFilter1SelectedValue = (String)HttpContext.Current.Session[HttpContext.Current.Session.SessionID + appRelativeVirtualPath + "ContactIDFilter1_Ajax"];
            if (MiscUtils.IsValueSelected(ContactIDFilter1SelectedValue)) {

              
                wc.iAND(LinkTableTable.ContactID, BaseFilter.ComparisonOperator.EqualsTo, ContactIDFilter1SelectedValue, false, false);
      ...
      LinkTableRecord rec = new LinkTableRecord();
                        ...
                        if (MiscUtils.IsValueSelected(recControl.ContactID)) {
                            rec.Parse(recControl.ContactID.SelectedItem.Value, LinkTableTable.ContactID);
                        }
        ...
        public virtual void SetContactIDLabel2()
                  {
        ...
        public virtual void SetContactIDFilter1()
        {
            
            this.PopulateContactIDFilter1(MiscUtils.GetSelectedValue(this.ContactIDFilter1,  GetFromSession(this.ContactIDFilter1)), 500);					
        ...
        // Get the filters' data for ContactIDFilter1.
                
        protected virtual void PopulateContactIDFilter1(string selectedValue, int maxItems)
                    
        {
            ...
            this.ContactIDFilter1.Items.Clear();
            WhereClause wc = this.CreateWhereClause_ContactIDFilter1();            
            			  			
            // Set up the WHERE and the ORDER BY clause by calling the CreateWhereClause_ContactIDFilter1 function.
            // It is better to customize the where clause there.
             
            // Setup the static list items        
            
                // Add the All item.
                this.ContactIDFilter1.Items.Insert(0, new ListItem(this.Page.GetResourceValue("Txt:All", "IPv5"), "--ANY--"));
            ...
            if (wc.RunQuery)
            {
                ...
                do
                {
                    ...
                    foreach (ContactsRecord itemValue in itemValues) 
                    {
                        ...
                        {
                            cvalue = itemValue.ContactID.ToString();
                            if (counter < maxItems && this.ContactIDFilter1.Items.FindByValue(cvalue) == null)
                            {
                                ...
                                ListItem dupItem = this.ContactIDFilter1.Items.FindByText(fvalue);
                                ...
                                ListItem newItem = new ListItem(fvalue, cvalue);
                                this.ContactIDFilter1.Items.Add(newItem);
            ...
            try
            {
                ...
                // Set the selected value.
                MiscUtils.SetSelectedValue(this.ContactIDFilter1, selectedValue);
            ...
            if (this.ContactIDFilter1.SelectedValue != null && this.ContactIDFilter1.Items.FindByValue(this.ContactIDFilter1.SelectedValue) == null)
                this.ContactIDFilter1.SelectedValue = null;
                           
        }
            
        public virtual WhereClause CreateWhereClause_ContactIDFilter1()
        {
            // Create a where clause for the filter ContactIDFilter1.
            // This function is called by the Populate method to load the items 
            // in the ContactIDFilter1DropDownList
        ...
        protected override void SaveControlsToSession()
        {
            ...
            this.SaveToSession(this.ContactIDFilter1, this.ContactIDFilter1.SelectedValue);
      ...
      this.SaveToSession("ContactIDFilter1_Ajax", this.ContactIDFilter1.SelectedValue);
        ...
        protected override void ClearControlsFromSession()
        {
            ...
            this.RemoveFromSession(this.ContactIDFilter1);
    ...
    this.ViewState["Page_Size"] = this.PageSize;
              ...
              
        {
         ...
         themeButtonFiltersButton.ArrowImage.ImageUrl = "../Images/ButtonExpandArrow.png";
            ...
            if (MiscUtils.IsValueSelected(ContactIDFilter1))
        ...
        protected virtual void ContactIDFilter1_SelectedIndexChanged(object sender, EventArgs args)
        {
            // Setting the DataChanged to True results in the page being refreshed with
            // the most recent data from the database.  This happens in PreRender event
            // based on the current sort, search and filter criteria.
...
#region "Helper Properties"
        ...
        public System.Web.UI.WebControls.DropDownList ContactIDFilter1 {
            get {
                return (System.Web.UI.WebControls.DropDownList)BaseClasses.Utils.MiscUtils.FindControlRecursively(this, "ContactIDFilter1");
            }
        }
        
        public System.Web.UI.WebControls.Literal ContactIDLabel2 {
            get {
                return (System.Web.UI.WebControls.Literal)BaseClasses.Utils.MiscUtils.FindControlRecursively(this, "ContactIDLabel2");
            }
        }
...
#region "Helper Functions"
        ...
        public virtual LinkTableTableControlRow[] GetSelectedRecordControls()
        {
            ...
            return (LinkTableTableControlRow[])(selectedList.ToArray(Type.GetType("IPv5.UI.Controls.EditProperties.LinkTableTableControlRow")));
...
// Base class for the PropertiesRecordControl control on the EditProperties page.
...
public class BasePropertiesRecordControl : IPv5.UI.BaseApplicationRecordControl
{
        ...
        public override void DataBind()
        {
            ...
            bool shouldResetControl = false;
            if (shouldResetControl) { }; // prototype usage to void compiler warnings
            LinkTableTableControl recLinkTableTableControl = (LinkTableTableControl)(MiscUtils.FindControlRecursively(this.Page, "LinkTableTableControl"));
        
          if (shouldResetControl || this.Page.IsPageRefresh)
          {
             recLinkTableTableControl.ResetControl();
          }
                  
        this.Page.SetControl("LinkTableTableControl");
      ...
      // To customize, override this method in PropertiesRecordControl.
         ...
         
        {
            ...
            HttpContext.Current.Session["QueryString in EditProperties"] = recId;
