...
// ShowPropertiesQuickList.aspx page.  The Row or RecordControl classes are the 
...
namespace IPv5.UI.Controls.ShowPropertiesQuickList
{
...
// Base class for the PropertiesTableControlRow control on the ShowPropertiesQuickList page.
...
public class BasePropertiesTableControlRow : IPv5.UI.BaseApplicationRecordControl
{
      ...
      this.Page.Response.Redirect(url);
        ...
        public virtual void ViewRowButton_Click(object sender, ImageClickEventArgs args)
        {
            ...
            string url = @"../Properties/GroupByPropertiesTable.aspx?Properties={PK}";
...
// Base class for the PropertiesTableControl control on the ShowPropertiesQuickList page.
...
public class BasePropertiesTableControl : IPv5.UI.BaseApplicationTableControl
{
       ...
       public BasePropertiesTableControl()
        ...
        protected virtual void Control_Init(object sender, System.EventArgs e)
        {
           ...
           // Setup the filter and search.
            ...
            {
                string initialVal = "";
                if  (this.InSession(this.CompanyNameFilter)) 				
                    initialVal = this.GetFromSession(this.CompanyNameFilter);
                
                else
                    
                    initialVal = EvaluateFormula("URL(\"CompanyName\")");
                ...
                if (initialVal != null && initialVal != "")		
                {
                    ...
                    string[] CompanyNameFilteritemListFromSession = initialVal.Split(',');
                    int index = 0;
                    foreach (string item in CompanyNameFilteritemListFromSession)
                    {
                        ...
                        {
                            this.CompanyNameFilter.Items.Add(item);
                            this.CompanyNameFilter.Items[index].Selected = true;
                            index += 1;
                        }
                    }
                    foreach (ListItem listItem in this.CompanyNameFilter.Items)
                    {
    ...
    // Setup default pagination settings.
       ...
       // Setup the sorting events.
            ...
            this.CompanyNameFilter.SelectedIndexChanged += new EventHandler(CompanyNameFilter_SelectedIndexChanged);
    ...
    public virtual void AddWarningMessageOnClick() {
        ...
        public virtual void ResetControl()
        {
            ...
            this.CompanyNameFilter.ClearSelection();
        ...
        public virtual WhereClause CreateWhereClause()
        {
            ...
            if (MiscUtils.IsValueSelected(this.CompanyNameFilter)) {
                        
                int selectedItemCount = 0;
                foreach (ListItem item in this.CompanyNameFilter.Items){
                    if (item.Selected) {
                        selectedItemCount += 1;
                ...
                foreach (ListItem item in this.CompanyNameFilter.Items){
                    ...
                    if (item.Selected){
                        filter.iOR(PropertiesTable.CompanyName, BaseFilter.ComparisonOperator.EqualsTo, item.Value, false, false);
                    }
                }
                wc.iAND(filter);
      ...
      foreach(BaseColumn col in cols)
      {
          ...
          search.iOR(col, BaseFilter.ComparisonOperator.Contains, MiscUtils.GetSelectedValue(this.SearchText, this.GetFromSession(this.SearchText)), true, false);
            ...
            if (MiscUtils.IsValueSelected(this.CompanyNameFilter) || this.InSession(this.CompanyNameFilter)){
      ...
      String CompanyNameFilterSelectedValue = (String)HttpContext.Current.Session[HttpContext.Current.Session.SessionID + appRelativeVirtualPath + "CompanyNameFilter_Ajax"];
            if (MiscUtils.IsValueSelected(CompanyNameFilterSelectedValue)) {

              
        if (CompanyNameFilterSelectedValue != null){
                        string[] CompanyNameFilteritemListFromSession = CompanyNameFilterSelectedValue.Split(',');
                        int index = 0;
                        WhereClause filter = new WhereClause();
                        foreach (string item in CompanyNameFilteritemListFromSession)
                        {
                            ...
                            {
                                filter.iOR(PropertiesTable.CompanyName, BaseFilter.ComparisonOperator.EqualsTo, item, false, false);
                                index += 1;
                            }
                        }
                        wc.iAND(filter);
      ...
      PropertiesRecord rec = new PropertiesRecord();
        ...
        public virtual void SetCompanyNameFilter()
        {
            
            ArrayList CompanyNameFilterselectedFilterItemList = new ArrayList();
            string CompanyNameFilteritemsString = null;
            if (this.InSession(this.CompanyNameFilter))
                CompanyNameFilteritemsString = this.GetFromSession(this.CompanyNameFilter);
            
            if (CompanyNameFilteritemsString != null)
            {
                string[] CompanyNameFilteritemListFromSession = CompanyNameFilteritemsString.Split(',');
                foreach (string item in CompanyNameFilteritemListFromSession)
                {
                    CompanyNameFilterselectedFilterItemList.Add(item);
                }
            }
              
            			
            this.PopulateCompanyNameFilter(MiscUtils.GetSelectedValueList(this.CompanyNameFilter, CompanyNameFilterselectedFilterItemList), 500);
                    ...
                    
        {
              ...
              // 1. Setup the static list items
                ...
                this.SortControl.Items.Add(new ListItem(this.Page.ExpandResourceValue("City {Txt:Ascending}"), "CityID Asc"));
              
                this.SortControl.Items.Add(new ListItem(this.Page.ExpandResourceValue("City {Txt:Descending}"), "CityID Desc"));
                ...
                this.SortControl.Items.Add(new ListItem(this.Page.ExpandResourceValue("Country {Txt:Ascending}"), "CountryID Asc"));
              
                this.SortControl.Items.Add(new ListItem(this.Page.ExpandResourceValue("Country {Txt:Descending}"), "CountryID Desc"));
                ...
                this.SortControl.Items.Add(new ListItem(this.Page.ExpandResourceValue("Region {Txt:Ascending}"), "RegionID Asc"));
              
                this.SortControl.Items.Add(new ListItem(this.Page.ExpandResourceValue("Region {Txt:Descending}"), "RegionID Desc"));
              
                this.SortControl.Items.Add(new ListItem(this.Page.ExpandResourceValue("Tenant Name {Txt:Ascending}"), "TenantName Asc"));
              
                this.SortControl.Items.Add(new ListItem(this.Page.ExpandResourceValue("Tenant Name {Txt:Descending}"), "TenantName Desc"));
        ...
        // Get the filters' data for CompanyNameFilter.
                
        protected virtual void PopulateCompanyNameFilter(ArrayList selectedValue, int maxItems)
                    
        {
            ...
            this.CompanyNameFilter.Items.Clear();
            WhereClause wc = this.CreateWhereClause_CompanyNameFilter();            
            			  			
            // Set up the WHERE and the ORDER BY clause by calling the CreateWhereClause_CompanyNameFilter function.
            // It is better to customize the where clause there.
             
            // Setup the static list items        
            
              // Add the Please Select item.
              this.CompanyNameFilter.Items.Insert(0, new ListItem(this.Page.GetResourceValue("Txt:PleaseSelect", "IPv5"), "--PLEASE_SELECT--"));
            ...
            if (wc.RunQuery)
            {
                ...
                values = PropertiesTable.GetValues(PropertiesTable.CompanyName, wc, orderBy, maxItems);
            ...
            string fvalue;
            if ( PropertiesTable.CompanyName.IsColumnValueTypeBoolean()) {
                    fvalue = cvalue;
                }else {
                    fvalue = PropertiesTable.CompanyName.Format(cvalue);
                }
                if (fvalue == null) {
                    fvalue = "";
                }
                ...
                ListItem dupItem = this.CompanyNameFilter.Items.FindByText(fvalue);
                ...
                this.CompanyNameFilter.Items.Add(newItem);
            ...
            try
            {
                ...
                // Set the selected value.
                MiscUtils.SetSelectedValue(this.CompanyNameFilter, selectedValue);
                

                if ((this.CompanyNameFilter.SelectedValue == null) || (selectedValue == null) || (selectedValue.Count == 0))
                    this.CompanyNameFilter.SelectedValue = "--PLEASE_SELECT--";
            ...
            if (this.CompanyNameFilter.SelectedValue != null && this.CompanyNameFilter.Items.FindByValue(this.CompanyNameFilter.SelectedValue) == null)
                this.CompanyNameFilter.SelectedValue = null;
                           
        }
            
        public virtual WhereClause CreateWhereClause_CompanyNameFilter()
        {
            // Create a where clause for the filter CompanyNameFilter.
            // This function is called by the Populate method to load the items 
            // in the CompanyNameFilterListBox
        ...
        protected override void SaveControlsToSession()
        {
            ...
            ArrayList CompanyNameFilterselectedFilterItemList = MiscUtils.GetSelectedValueList(this.CompanyNameFilter, null);
            string CompanyNameFilterSessionString = "";
            if (CompanyNameFilterselectedFilterItemList != null){
                foreach (string item in CompanyNameFilterselectedFilterItemList){
                    CompanyNameFilterSessionString = String.Concat(CompanyNameFilterSessionString ,"," , item);
                }
            }
            this.SaveToSession(this.CompanyNameFilter, CompanyNameFilterSessionString);
        ...
        protected  void SaveControlsToSession_Ajax()
        {
            ...
            this.SaveToSession("CompanyNameFilter_Ajax", CompanyNameFilterSessionString);
      ...
      this.SaveToSession("SearchText_Ajax", this.SearchText.Text);
        ...
        protected override void ClearControlsFromSession()
        {
            ...
            this.RemoveFromSession(this.CompanyNameFilter);
    ...
    this.ViewState["Page_Size"] = this.PageSize;
              ...
              
        {
                    ...
                    this.ImportButton.PostBackUrl = "../Shared/SelectFileToImport.aspx?TableName=Properties" ;
              ...
              
        {
              ...
              try
              {
                    string url = "../Properties/AddProperties.aspx?TabVisible=False&SaveAndNewVisible=False";
                    ...
                    url = url + "&RedirectStyle=" + (this.Page as BaseApplicationPage).Encrypt("Popup") + "&Target=" + (this.Page as BaseApplicationPage).Encrypt(MiscUtils.FindControlRecursively(this, "PropertiesTableControl_PostbackTracker").ClientID);                           
                                
                string javascriptCall = "";
                
                    javascriptCall = "initializePopupPage(document.getElementById('" + MiscUtils.FindControlRecursively(this, "PropertiesTableControl_PostbackTracker").ClientID + "'), '" + url + "', true, event);";                                      
        ...
        public virtual void PDFButton_Click(object sender, ImageClickEventArgs args)
        {
            ...
            try {
                ...
                report.SpecificReportFileName = Page.Server.MapPath("ShowPropertiesQuickList.PDFButton.report");
                // report.Title replaces the value tag of page header and footer containing ${ReportTitle}
                report.Title = "Properties";
                // If ShowPropertiesQuickList.PDFButton.report specifies a valid report template,
                            ...
                            // The 4th parameter represent the maximum length of the data value being shown
                             ...
                             if (BaseClasses.Utils.MiscUtils.IsNull(record.CityID)){
                                 ...
                                 if (_isExpandableNonCompositeForeignKey &&  ( _DFKA  != null)  &&  PropertiesTable.CityID.IsApplyDisplayAs){
                                     ...
                                     report.AddData("${CityID}", record.Format(PropertiesTable.CityID), ReportEnum.Align.Left, 100);
                                 }
                             }
                             if (BaseClasses.Utils.MiscUtils.IsNull(record.RegionID)){
                                 ...
                                 if (_isExpandableNonCompositeForeignKey &&  ( _DFKA  != null)  &&  PropertiesTable.RegionID.IsApplyDisplayAs){
                                     ...
                                     report.AddData("${RegionID}", record.Format(PropertiesTable.RegionID), ReportEnum.Align.Left, 100);
                                 }
                             }
                             if (BaseClasses.Utils.MiscUtils.IsNull(record.CountryID)){
                                 ...
                                 if (_isExpandableNonCompositeForeignKey &&  ( _DFKA  != null)  &&  PropertiesTable.CountryID.IsApplyDisplayAs){
                                     ...
                                     report.AddData("${CountryID}", record.Format(PropertiesTable.CountryID), ReportEnum.Align.Left, 100);
                                 }
                             }
        ...
        public virtual void WordButton_Click(object sender, ImageClickEventArgs args)
        {
            ...
            try {
                ...
                report.SpecificReportFileName = Page.Server.MapPath("ShowPropertiesQuickList.WordButton.word");
                // report.Title replaces the value tag of page header and footer containing ${ReportTitle}
                report.Title = "Properties";
                // If ShowPropertiesQuickList.WordButton.report specifies a valid report template,
                // AddColumn methods will generate a report template.
                // Each AddColumn method-call specifies a column
                // The 1st parameter represents the text of the column header
                // The 2nd parameter represents the horizontal alignment of the column header
        ...
        protected virtual void SortControl_SelectedIndexChanged(object sender, EventArgs args)
        {
                  ...
                  foreach (BaseClasses.Data.BaseColumn ColumnNam in PropertiesTable.GetColumns())
                  {
                ...
                OrderByItem sd = this.CurrentSortOrder.Find(PropertiesTable.GetColumnByName(SelVal1));
                if (sd == null || this.CurrentSortOrder.Items != null)
                {
                // First time sort, so add sort order for Discontinued.
                if (PropertiesTable.GetColumnByName(SelVal1) != null)
                {
                  this.CurrentSortOrder.Reset();
                }
                ...
                if ((this.CurrentSortOrder).GetType() == typeof(GeoOrderBy)) this.CurrentSortOrder = new OrderBy(true, false);
                  ...
                  if (SelVal1 != "--PLEASE_SELECT--" && PropertiesTable.GetColumnByName(SelVal1) != null)
                  {
                    if (words1[words1.Length - 1].Contains("ASC"))
                  {
                  this.CurrentSortOrder.Add(PropertiesTable.GetColumnByName(SelVal1),OrderByItem.OrderDir.Asc);
                    }
                    else
                    {
                      if (words1[words1.Length - 1].Contains("DESC"))
                  {
                  this.CurrentSortOrder.Add(PropertiesTable.GetColumnByName(SelVal1),OrderByItem.OrderDir.Desc );
                      }
                    }
                  }
        ...
        protected virtual void CompanyNameFilter_SelectedIndexChanged(object sender, EventArgs args)
        {
            // Setting the DataChanged to True results in the page being refreshed with
            // the most recent data from the database.  This happens in PreRender event
            // based on the current sort, search and filter criteria.
...
#region "Helper Properties"
        ...
        public System.Web.UI.WebControls.ListBox CompanyNameFilter {
            get {
                return (System.Web.UI.WebControls.ListBox)BaseClasses.Utils.MiscUtils.FindControlRecursively(this, "CompanyNameFilter");
            }
        }
...
#region "Helper Functions"
        ...
        public virtual PropertiesTableControlRow[] GetSelectedRecordControls()
        {
            ...
            return (PropertiesTableControlRow[])((new ArrayList()).ToArray(Type.GetType("IPv5.UI.Controls.ShowPropertiesQuickList.PropertiesTableControlRow")));
